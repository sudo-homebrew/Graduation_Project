...
#include "nav2_custom_planner/my_navfn_planner.hpp"
...
{
  RCLCPP_INFO(
    logger_, "Destroying plugin %s of type MyNavfnPlanner",
    name_.c_str());
}
...
{
  RCLCPP_INFO(
    logger_, "Activating plugin %s of type MyNavfnPlanner",
    name_.c_str());
...
{
  RCLCPP_INFO(
    logger_, "Deactivating plugin %s of type MyNavfnPlanner",
    name_.c_str());
...
{
  RCLCPP_INFO(
    logger_, "Cleaning up plugin %s of type MyNavfnPlanner",
    name_.c_str());
  planner_.reset();
}
...
// bool
// MyNavfnPlanner::validPointPotential(const geometry_msgs::msg::Point & world_point)
// {
//   return validPointPotential(world_point, tolerance_);
// }

// bool
// MyNavfnPlanner::validPointPotential(
...
PLUGINLIB_EXPORT_CLASS(nav2_custom_planner::MyNavfnPlanner, nav2_core::GlobalPlanner)