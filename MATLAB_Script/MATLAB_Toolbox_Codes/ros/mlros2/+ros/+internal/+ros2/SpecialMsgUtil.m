    ...
    methods(Static)
        ...
        % Validate input message struct
            ...
            if isempty(coder.target)
                ...
                encoding = coder.internal.getParameterValue(pStruct.Encoding,'rgb8',varargin{2:end});
        ...
        % Validate Input argument
            ...
            fieldData = ros.internal.ros2.SpecialMsgUtil.readFieldFromData(msg.data, ...
                                                                           byteIdx, pointIdxIsValid, mlType, count);

            % Reshape the output if requested by the user
            if reshapeOutput && msg.height ~= 1
                if count > 1
                    fieldData = reshape(fieldData, msg.width, msg.height, count);
                    fieldData = permute(fieldData, [2 1 3]);
                else
                    fieldData = reshape(fieldData, msg.width, msg.height).';
                end
            end
        end
        ...
        %TypeConversion - Object handling ROS <--> MATLAB type conversions
            ...
            xyzTemp = ...
                [ros.internal.ros2.SpecialMsgUtil.readFieldFromData(msg.data, byteIdx, pointIdxIsValid, xMlType,1), ...
                 ros.internal.ros2.SpecialMsgUtil.readFieldFromData(msg.data, byteIdx + yOff, pointIdxIsValid, yMlType,1), ...
                 ros.internal.ros2.SpecialMsgUtil.readFieldFromData(msg.data, byteIdx + zOff, pointIdxIsValid, zMlType,1)];
        ...
        % Validate input message data
            ...
            [byteIdx, pointIdxIsValid] = ros.internal.ros2.SpecialMsgUtil.getByteIndexForField(msg, rgbIdx, pointIndices, 1);
