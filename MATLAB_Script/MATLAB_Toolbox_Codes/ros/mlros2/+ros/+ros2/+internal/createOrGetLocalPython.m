...
%
% Note: The Python environment created by this function is used by ROS Toolbox
...
% [~, ~, pyenvDir] = ros.ros2.internal.createOrGetLocalPython(true)
%
% See also pyenv, setenv.
...
%   Copyright 2019-2022 The MathWorks, Inc.
    ...
    % Calculate Python venv folder name
    pyenvDir = fullfile(venvRoot,'ros2',computer('arch'),'venv');
    if ispc
    ...
    versionval = ros.internal.utilities.getVersionVal(pe.Version);
    
    % version distributed with MATLAB was tested with Python 3.9
    requiredMinVersion = '3.9';
    requiredMaxVersion = '3.9';
    requiredMinVersionVal = ros.internal.utilities.getVersionVal(requiredMinVersion);
    requiredMaxVersionVal = ros.internal.utilities.getVersionVal(requiredMaxVersion);
    if (versionval < requiredMinVersionVal) || (versionval > requiredMaxVersionVal)
        error(message('ros:utilities:util:RequirePython', requiredMinVersion));
    end
    
    % In MAC and Linux, executable path might be a symbolic link. We need to find the
    % real path
    ...
    if ~isfile(localPythonPath)
        error(message('ros:utilities:util:ErrorCreatingPython3VenvPyexecNotFound',...
            localPythonPath,'ros.ros2.internal.createOrGetLocalPython(true)'));
    end

    % Find the last package that will be installed
    % If found, assuming all other packages installed properly
    if ispc
        colcondir = fullfile(pyenvDir,'Lib','site-packages','numpy');
        foundColcon = isfolder(colcondir);
    else
        startSearch = fullfile(pyenvDir,'lib');
        startSearch = replace(startSearch, ' ', '\ ');
        [stat, res] = system(['find ' startSearch ' -type d -name numpy -print']);
        assert(stat == 0, res);
        foundColcon = ~isempty(res);
    end

    %if not found install packages
    if ~foundColcon
        dotprinter = ros.internal.DotPrinter('ros:utilities:util:AddingReqdPackages'); %#ok<NASGU>
        packagesToInstall = ' argcomplete flake8 flake8-blind-except flake8-class-newline flake8-comprehensions flake8-docstrings flake8-import-order flake8-quotes pytest pytest-cov pytest-repeat pytest-rerunfailures colcon-common-extensions lark-parser numpy netifaces';
        % key is to protect filepaths with spaces
        if ispc
            cmdToRun = ['"' activatePath '" && "' localPythonPath '" -m pip install --force --no-index --find-links="' fullfile(matlabroot,'sys','ros2','share','python') '"' packagesToInstall];
        else
            %for some reason activate command does not have exec bit set
            [stat, res] = system(['chmod +x ' replace(activatePath,' ','\ ')]);
            assert(stat == 0, res);
            cmdToRun1 = ['sh -c "PATH=' fileparts(pyexec) ' ; ' replace(activatePath,' ','\ ') ' ; ' replace(localPythonPath,' ','\ ') ' -m pip install --no-index --find-links=' fullfile(replace(matlabroot,' ','\ '),'sys','ros2','share','python') ' pip==21.2.2 setuptools==57.0.0' '"'];
            cmdToRun = ['sh -c "PATH=' fileparts(pyexec) ' ; ' replace(activatePath,' ','\ ') ' ; ' replace(localPythonPath,' ','\ ') ' -m pip install --no-index --find-links=' fullfile(replace(matlabroot,' ','\ '),'sys','ros2','share','python') packagesToInstall '"'];
