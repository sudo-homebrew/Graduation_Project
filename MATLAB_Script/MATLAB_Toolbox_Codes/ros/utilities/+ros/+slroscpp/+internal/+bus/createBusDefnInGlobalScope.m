...
%   Copyright 2014-2021 The MathWorks, Inc.
    ...
    if nullModel
        ...
        varlenArrayStore = ros.slros.internal.bus.VarlenArraySizeStore(model, 'BusUtilityObject', ros.slroscpp.internal.bus.Util);
        truncateAction = varlenArrayStore.getTruncateAction();
    end

    requiredBuses = ros.slros.internal.bus.getBusDefnForROSMsg(emptyRosMsg, model, @ros.slroscpp.internal.bus.defineSimulinkBus);
    ...
    for i = 1:numel(requiredBuses)
        ...
        busname = ros.slros.internal.bus.Util.rosMsgTypeToBusName(msgType, model);
        ...
        if varlenInfo.hasVarLenArrayProperties()
            ...
            ros.slros.internal.assigninGlobalScope(model, busname, updatedBus);
            createVarlenInfoBus = true;
        else
            % no variable-length arrays
            ros.slros.internal.assigninGlobalScope(model, busname, bus);
        end
    end

    if createVarlenInfoBus
        ros.slros.internal.bus.Util.createVarlenInfoBusIfNeeded(model);
    end
