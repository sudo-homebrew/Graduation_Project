...
stdSrvRoot = {fullfile(matlabroot,'sys','ros1',computer('arch'),'ros1','share'), ...
    fullfile(matlabroot,'toolbox','ros','mlroscpp','custom_messages',...
    'share')};
msgDefn.msgInfo = ros.internal.ros.getServiceInfo(msgDefn.MessageType,srvType,type);
...
msgNameForClass = msgInfo.msgName;
if isfield(msgInfo, 'srvName')
    msgNameForClass = msgInfo.srvName;
end
order = {};
matpath = {};
msgpath = {};
        ...
        if ~isempty(field.fieldNames) || (isfield(field,'MessageType') && isempty(field.fieldNames))
            ...
            if field.count ~= 0 || ~isequal(field.msgInfo, msgInfo)
                ...
                field.endSubLine{1} = sprintf('%soutArray[ctr]["%s"] = %soutArray;',...
                    prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldNames{i}, msgNameForClass), fldPrefix);
            end
...
if ~isnan(fldCount) && fldCount < 1
    ...
    if isequal(msgpath, fieldName)
        ...
        for i = 1:nelemMatPath-1
            ...
            field.pubLine{end+1} = sprintf('const matlab::data::StructArray _%s_arr = %s["%s"];', ...
                varPrefix, valArr, ros.internal.utilities.convertROSFieldsToClassFields(matpathSplit{i}, msgNameForClass));
    ...
    field.pubLine{end+1} = sprintf('const matlab::data::StructArray %s%s_arr = %s["%s"];', ...
        prefixForVar, fieldName, valArr, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass));
else
...
if isnan(field.count) || field.count ~= 0
    ...
    field.subLine{end+1} = sprintf('%soutArray[ctr]["%s"] = msgClassPtr_%s->get_arr(factory,&currentElement_%s[0],loader,currentElement_%s.size());',...
        prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), fieldName, fieldName, fieldName);
    ...
    field.subLine{end+1} = sprintf('%soutArray[ctr]["%s"] = msgClassPtr_%s->get_arr(factory, &currentElement_%s, loader);',...
        prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), fieldName, fieldName);
    ...
    if fldCount ~= 0
        ...
        if ~isnan(field.count) && field.count < 1
            ...
            field.subLine{end+1} = sprintf('\t%soutArray[idx]["%s"] = factory.createCharArray(std::string(1, val[idx].%s));',...
                prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), fieldName);
        else
...
elseif isequal(field.MLdataType,'string')
    ...
    if fldCount ~= 0
        ...
        if ~isnan(field.count) && field.count < 1
            ...
            field.subLine{end+1} = sprintf('\t%soutArray[idx]["%s"] = %s%soutCell;',...
                prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), prefixForVar, fieldName);
        end
    else
        %parent is a single element, therefore outArray[ctr][fieldname] = ...
        if ~isnan(field.count) && field.count < 1
    ...
    field.pubLine{end+1} = sprintf('const matlab::data::TypedArray<%s> %s%s_arr = %s["%s"];', ...
        field.CPPdataType, prefixForVar, fieldName, valArr, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass));
        ...
        if ~isnan(field.count) && field.count < 1
            if ~isnan(field.constantValue) || isequal(field.CPPdataType,'bool')
                ...
                field.subLine{end+1} = sprintf('\t%soutArray[idx]["%s"] = factory.createScalar(static_cast<%s>(val[idx].%s));',...
                    prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), field.CPPdataType, fieldName);
            else
            ...
            if isequal(field.CPPdataType,'bool')
                ...
                field.subLine{end+1} = sprintf('\t%soutArray[idx]["%s"] = factory.createArray<std::vector<%s>::const_iterator, %s>({%s_%s.size(),1}, %s_%s.begin(), %s_%s.end());',...
                    prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), field.CPPdataType, field.CPPdataType, fieldName, field.CPPdataType, fieldName, field.CPPdataType, fieldName, field.CPPdataType);
            else
                field.subLine{end+1} = sprintf('\t%soutArray[idx]["%s"] = factory.createArray<%s::_%s_type::const_iterator, %s>({val[idx].%s.size(),1}, val[idx].%s.begin(), val[idx].%s.end());',...
                    prefixForVar, ros.internal.utilities.convertROSFieldsToClassFields(fieldName, msgNameForClass), msgInfo.msgCppClassName, fieldName, field.CPPdataType, fieldName, fieldName, fieldName);
            end
        end
    else
        %parent is a single element, therefore outArray[ctr][fieldname] = ...
...
get_arr
    ...
    Instance).
        ...
        message of the same package, so we dont need to create class_loader instance but 
        just need to create a static instance of the diagnostic_msgs/DiagnosticStatus and call its get_arr).
...
msgDefn.msgFields.header
     ...
     constantValue: NaN
      defaultValue: NaN
           ...
           msgInfo: [1x1 struct]
        fieldNames: {3x1 cell}
    subConstructor: 'auto _headeroutArray = factory.createStructArray({size,1},{"MessageType","Seq","Stamp","FrameId"});'
...
msgDefn.msgFields.status
     ...
     constantValue: NaN
      defaultValue: NaN
           ...
           msgInfo: [1x1 struct]
        fieldNames: {9x1 cell}
    subConstructor: 'auto _statusoutArray = factory.createStructArray({size,1},{"MessageType","OK","WARN","ERROR","STALE","Level","Name","Message","HardwareId","Values"});'
