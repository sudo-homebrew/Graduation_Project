...
void TGT_libCreate(void** device_,
                   char* err,
                   NetworkInputOrOutput io,
                   const char* localURL,
                   int localPort,
...
void TGT_libCreate(void** device_,
    ...
    if ((device->remotesockaddr.sin_addr.s_addr == INADDR_BROADCAST) ||
        ...
        ret = setsockopt(((TgtSockStructT*)device)->sockfd, SOL_SOCKET, SO_BROADCAST, &broadcast_en,
                         sizeof(broadcast_en));
        if (ret < 0) {
    ...
    if (io == NETWORK_INPUT) {
        ret = setsockopt(((TgtSockStructT*)device)->sockfd, SOL_SOCKET, SO_RCVBUF, &bufferSize,
                         sizeof(bufferSize));
    } else if (io == NETWORK_OUTPUT) {
        ret = setsockopt(((TgtSockStructT*)device)->sockfd, SOL_SOCKET, SO_SNDBUF, &bufferSize,
                         sizeof(bufferSize));
    } else {
        ret = -1;
    ...
    if (blockingTime == 0) {
        ...
        setsockopt(((TgtSockStructT*)device)->sockfd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv));
    }
...
void TGT_libOutputs(void* device, char* err, void* src, int* nSamples) {
    ...
    if (device) {
        n = recvfrom(((TgtSockStructT*)device)->sockfd, src,
                     ((TgtSockStructT*)device)->dataTypeSize * (*nSamples), 0, NULL, NULL);
        if (n > 0) {
