function IMM = initekfimm(Detection)
...
EKF{1}  = initcvekf(Detection);
EKF{2}  = initcaekf(Detection);
EKF{3}  = initctekf(Detection);

for i =1:3
   setMeasurementSizes(EKF{i}, n, n);
   EKF{i}.MeasurementNoise  = measurementNoise;
end

% Assign the function handle of model conversion function to mdlConv.
mdlConv = @switchimm;
% Creating the IMM filter object.
IMM = trackingIMM('TrackingFilters',EKF,...
    'ModelConversionFcn',mdlConv,'TransitionProbabilities',cast(0.9,classToUse));