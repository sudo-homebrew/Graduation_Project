function zCorners = ctrectcorners(state, varargin)
...
if numel(varargin) > 0
    ...
    if isstruct(candidateStruct)
        argsinCell = matlabshared.tracking.internal.fusion.measmodelsvalidateoptionalstruct(funcName, class(stateCol), candidateStruct);
...
[isRect, sensorpos, sensorvel, localAxes, hasAz, hasEl, hasVel, hasRange] = ...
    matlabshared.tracking.internal.fusion.measmodelsvalidateoptionalinput(funcName, class(stateCol), argsinCell{:});


zCorners = fusion.internal.RectangularMeasurementUtilities.computeCornerMeasurements(stateCol, isRect, sensorpos, sensorvel, localAxes, hasAz, hasEl, hasVel, hasRange);
