...
%     ASSIGNAUCTION(COSTMATRIX, COSTOFNONASSIGNMENT) assigns rows to
%   columns based on the COSTMATRIX using the forward/reverse Auction
%   assignment algorithm which finds a sub-optimal solution to the global
%   nearest neighbor (GNN) assignment problem, where each column is
%   assigned to a row in a way that minimizes the total cost, assuming
%   their cost is less than the COSTOFNONASSIGNMENT parameter, which serves
...
% title('Auction Assignment of Detections to Tracks');
...
%
% See also: assignmunkres, assignjv.
...
[rowSoln, colSoln] = fusion.internal.assignment.lapAuctionFwdRev(costMatrix);
