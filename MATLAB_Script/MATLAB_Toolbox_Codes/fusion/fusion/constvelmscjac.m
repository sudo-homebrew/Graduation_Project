function [jacState,jacNoise] = constvelmscjac(state,vNoise,varargin)
...
[stateCol,vNoise,dT,obsManuever,hasZStates] = ...
    fusion.internal.parseAndValidateMSCInputs(state,vNoise,mfilename,varargin{:});

% Convert MSC to Cartesian State
cartState = fusion.internal.mscToCartesian(stateCol);

% Predict cartesian state using non-maneuvering target motion model
G1d = [dT^2/2;dT];
B = blkdiag(G1d,G1d,G1d);
cartStateT = constvel(cartState,dT) + B*vNoise - obsManuever;

% Get Jacobian dmsc(t+1)/dcart(t+1)
jacMSCTToCartT = fusion.internal.cartesianToMSCGradient(cartStateT);

% Get Jacobian dcart(t+1)/dcart(t)
jacCartT2Cart = constveljac(cartState,dT);

% Get Jacobian dcart(t)/dmsc(t)
jacCartToMSC = fusion.internal.mscToCartesianGradient(stateCol);

% Jacobian = dmsc(t+1)/dmsc(t)
jacTotal = jacMSCTToCartT*jacCartT2Cart*jacCartToMSC;

if ~hasZStates
    jacState = jacTotal([1:2,5:6],[1:2,5:6]);
else
    jacState = jacTotal;
end

jacTotalNoise = jacMSCTToCartT*B;

if ~hasZStates
    jacNoise = jacTotalNoise([1:2,5:6],1:2);
else
    jacNoise = jacTotalNoise;
end
