function PF = initcapf(Detection)
...
% 4. If the state is unobservable, use uniform initialization along
% unobservable states 
...
if isRect
    ...
    if ~invalidDet
        PF = trackingPF(@constacc,@cameas,state,'NumParticles',numParticles,...
        ...
        PF = trackingPF(@constacc,@cameas,state,'NumParticles',numParticles,...
            'HasAdditiveProcessNoise', false,'StateCovariance',stateCov,...
            'ProcessNoise',Q);
    end
else
...
if ~invalidDet
    PF = trackingPF(@constacc,@cameas,zeros(9,1),'NumParticles',numParticles,...
...
PF.Particles(4,:) = posParticles(2,:);
PF.Particles(7,:) = posParticles(3,:);
PF.Particles(2,:) = velParticles(1,:);
PF.Particles(5,:) = velParticles(2,:);
PF.Particles(8,:) = velParticles(3,:);
end
