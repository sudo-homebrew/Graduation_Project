...
class setLinkAngularVelocityTest : public testing::Test {
  public:
    ...
    void SetUp() {
        ...
        /// Launch SetLinkAngularVelocityMsgHandler message handler
        ...
        m_worldUpdateStartEventConnection = gazebo::event::Events::ConnectWorldUpdateBegin(
            std::bind(&setLinkAngularVelocityTest::onWorldUpdateStart, this));
...
*/
TEST_F(setLinkAngularVelocityTest, testSetLinkAngularVelocitySuccess) {
    m_world->Run();
    ...
    // set link angular velocity for 1 second
    mw::internal::robotics::gazebotransport::Packet reply =
        clientSetLinkAngularVelocity(modelName, linkName, velocity_values, 1, 0);
    ASSERT_EQ(reply.status(),
              mw::internal::robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);

    // step simulator few steps (10 steps ) and check for link angular velocity
    m_world->Step(10);
    EXPECT_NEAR(link->RelativeAngularVel()[0], velocity_values[0], 1e-1);
    EXPECT_NEAR(link->RelativeAngularVel()[1], velocity_values[1], 1e-1);
    EXPECT_NEAR(link->RelativeAngularVel()[2], velocity_values[2], 1e-1);
}
...
/*
 ...
 Also, it tests the client successfully gets the NO Error message
*/
TEST_F(setLinkAngularVelocityTest, testSetLinkAngularVelocityTwiceSuccess) {
    m_world->Run();
    m_world->SetPaused(true);
    m_world->ResetTime();
    ...
    // set link angular velocity for 1 second
    mw::internal::robotics::gazebotransport::Packet reply =
        clientSetLinkAngularVelocity(modelName, linkName, velocity_values0, 1, 0);
    ASSERT_EQ(reply.status(),
              mw::internal::robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);

    // step simulator few steps (10 steps ) and check for link angular velocity
    m_world->Step(10);
    EXPECT_NEAR(link->RelativeAngularVel()[0], velocity_values0[0], 1e-1);
    EXPECT_NEAR(link->RelativeAngularVel()[1], velocity_values0[1], 1e-1);
    EXPECT_NEAR(link->RelativeAngularVel()[2], velocity_values0[2], 1e-1);

    // change velocity values
    double velocity_values1[3] = {2.0, 1.0, 3.0};

    // set link angular velocity for 1 second
    reply = clientSetLinkAngularVelocity(modelName, linkName, velocity_values1, 1, 0);
    ASSERT_EQ(reply.status(),
              mw::internal::robotics::gazebotransport::Packet_CoSimError::Packet_CoSimError_NONE);

    // step simulator few steps (10 steps ) and check for updated link angular velocity
    m_world->Step(10);
    EXPECT_NEAR(link->RelativeAngularVel()[0], velocity_values1[0], 1e-1);
    EXPECT_NEAR(link->RelativeAngularVel()[1], velocity_values1[1], 1e-1);
    EXPECT_NEAR(link->RelativeAngularVel()[2], velocity_values1[2], 1e-1);
}
 ...
 */
TEST_F(setLinkAngularVelocityTest, testInValidModelName) {
    double velocity_values[3] = {1.0, 2.0, 1.0};

    mw::internal::robotics::gazebotransport::Packet reply =
        clientSetLinkAngularVelocity("unt_box", "link", velocity_values, 1, 0);
 ...
 */
TEST_F(setLinkAngularVelocityTest, testInValidLinkName) {
    double velocity_values[3] = {1.0, 2.0, 1.0};

    mw::internal::robotics::gazebotransport::Packet reply =
        clientSetLinkAngularVelocity("unit_box", "lik", velocity_values, 1, 0);
