...
#ifndef TESTPOSEMSGHANDLER_HPP
#define TESTPOSEMSGHANDLER_HPP
#include <memory>
#include <cstdint>
#include <string>
...
typedef const boost::shared_ptr<const TestPose> TestPoseCustomPtr;
...
class TestPoseSubscriber {
  public:
    ...
    */
    explicit TestPoseSubscriber(std::string const& topicName)
    ...
    /// Destructor
    virtual ~TestPoseSubscriber() {
    }

    // subscriber callback which subscribe custom message from specific topic
    // and stores in the custom message container
    void OnUpdate(robotics::gazebotransport::TestPoseCustomPtr& _msg) {
        {
    ...
    void InitImpl(gazebo::transport::NodePtr node) {
        if (!m_subscriberPtr) {
            m_subscriberPtr = node->Subscribe(m_topicName, &TestPoseSubscriber::OnUpdate, this);
        }
    }
...
class TestPoseMsgHandler : public CustomMsgHandler {
  public:
    ...
    @param customContainer          Custom message storage instance
    */
    explicit TestPoseMsgHandler(gazebo::transport::NodePtr node);
    ...
    std::pair<std::shared_ptr<TestPoseSubscriber>, bool> insert(
        std::string const& topic_name,
        std::shared_ptr<TestPoseSubscriber> subsciberPointer);

    // return subscriber instance stored in map based on topic name
    std::shared_ptr<TestPoseSubscriber> find(std::string const& topic);
  ...
  private:
    ...
    std::map<std::string, std::shared_ptr<TestPoseSubscriber>> m_subscriberPtrMap;
