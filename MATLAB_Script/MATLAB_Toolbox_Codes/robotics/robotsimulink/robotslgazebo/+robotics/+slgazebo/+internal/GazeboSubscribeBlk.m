classdef GazeboSubscribeBlk < matlab.System
    %This function is for internal use only. It may be removed in the future.

    %GazeboSubscribe reads custom message from Gazebo and output its content
    ...
    methods(Access = protected)
        ...
        % Initialize / reset discrete-state properties
            ...
            if coder.target('MATLAB')
                ...
                if ~obj.Reader.subscribeCustomIfNeeded(obj.TopicType, obj.TopicName)
                    error(message('robotics:robotslgazebo:subscribeblock:SubscribeFailed', obj.TopicName));
                end

                if(exist('importGazeboCustomLoadLibrary.m','file'))
                    importGazeboCustomLoadLibrary;
                else
                    error(message('robotics:robotslgazebo:subscribeblock:CustomDataMissing'));
                end
        ...
        % Calculate output
            ...
            if coder.target('MATLAB')
                ...
                [isNew, result] = obj.Reader.readCustom(obj.TopicType, obj.TopicName);
                ...
                result = robotics.slgazebo.internal.util.convertToDouble(result);
