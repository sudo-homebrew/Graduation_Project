...
%   Copyright 1991-2005 MUSYN Inc. and The MathWorks, Inc.
 ...
 rub(M,K,I,J,opt,numits,contol,Kc,Kr,Ic,Ir,Jc,Jr,Fc,Fr,kc,kr,ic,ir,...
     jc,jr,fc,fr,sc,sr,csc,csr,csF,nK,nI,nJ,nF,nc,nr,kcK,icI,jcJ,jrJ,...
     fcF,dcb,drib,y,b,ldc,ldri,lg,lal)
...
%%%%%%%%%%  balancing
if exist('drib','var'),   dcbold = dcb;   dribold = drib;
...
if opt.multiplelower
   for rreepp=1:opt.multiplelowertimes
      b = crand(csr(4),1);    b = b/(norm(b) + 10*eps);
      y = crand(csr(4),1);    y = y/(norm(y) + 10*eps);
      [lbtry,ytry,btry,perttry,warflagtry] = rublow(M,nK,Kc,Kr,Ic,Ir,Jc,...
        Jr,kcK,icI,jcJ,jrJ,ic,ir,jc,jr,sc,sr,y,b,numits(2),warflag);
      % lb*scale
      if lbtry>lb
         lb = lbtry; pert = perttry; y = ytry; b = btry; warflag=warflagtry;
      end
   end
end
