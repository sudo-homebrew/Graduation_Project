...
if length(ind)>5 || isempty(ind),	return,				end
u    = u(LKc,ind);
s   = s(ind,ind);
v   = v(LKr,ind);
...
grad = -real(((abs(u).^2 + abs(v).^2).*ginv)*s + 1i*conj(u).*v.*galf);
if norm(grad,'fro')<10*eps, 		return,			end
grad2 = grad'*grad;
%XXX GJB changed 15jan04
% the equation below was 1e-19 and caused problems, should make this
%  value larger to eliminate problems caused by mldivide
%if abs(det(grad2)) > 1e-20
if abs(det(grad2)) > 1e-14
   gdir = -grad*(grad2\(diag(s)-0.90))*10;	% because Mathworks lies
else
   return
end
radi = real( sig + 1i*u(:,1)'*(galf(:,1).*v(:,1).*g) )/alpha;
if abs(radi)<10*eps
   % sigmax is independent of alpha
   junk = -grad(:,1)'*gdir;
   if junk<10*eps || ~isfinite(norm(junk)), return,		end
   scale = -sig/100/junk;
else
   arad = grad(:,1)/radi;
   junk = -arad'*gdir;
...
jg(LKc,LKr) = diag(1i*g);
gamc(LKc)  = (1+g.*g).^(-1/4);
gamr(LKr)  = gamc(LKc);
if exist('arad','var'), dal = arad'*delg/2; else, dal = bnds(1)/50; end
