function [gam_opt,acp,bcp,ccp,dcp,acl,bcl,ccl,dcl,ak,bk1,bk2,ck1,ck2,dk11,dk12,dk21,dk22] = hinfopt(varargin)
...
while relerr > tol
    ...
    [tacp,tbcp,tccp,tdcp,tacl,tbcl,tccl,tdcl,hinfo,tak,tbk1,tbk2,tck1,tck2,tdk11,tdk12,tdk21,tdk22] = hinf(...
    ...
    if gamaflg + RHP_cl == 0
       ...
       [ak,bk1,bk2,ck1,ck2,dk11,dk12,dk21,dk22]=deal(tak,tbk1,tbk2,tck1,tck2,tdk11,tdk12,tdk21,tdk22);
       gam_opt = rhoi; p_opt = p;
    else
       rho_h = rhoi;
       xflag = 1;
    end
    if verbose, disp([sprintf('%4.0f',p),sprintf('%12.4e',rhoi),'   ', D11flag,'     ',Pexist,'     ',Pflag,'    ',Sexist,'     ',Sflag,'      ',PSflag,'      ',CLflag]), end
    if xflag == 0
       rhoi = 2*rhoi;
    else
       if yflag == 0
...
if xsflag
   ...
   if nag2 >3
      ccp=mktito(ss(ak,[bk1,bk2],[ck1;ck2],[dk11,dk12;dk21,dk22]),size(dk22,1),size(dk22,2));
   end
end
%
