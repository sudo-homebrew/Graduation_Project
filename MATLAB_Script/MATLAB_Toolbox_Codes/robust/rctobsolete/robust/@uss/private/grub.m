%function [bnds,pert,dc,dri,dcb,drib,dcd,drid,g,y,b,sens] = ...
%     grub(M,K,I,J,opt,numits,contol,Kc,Kr,Ic,Ir,Jc,Jr,Fc,Fr,kc,kr,
%         ic,ir,jc,sr,fc,fr,sc,sr,csc,csr,csF,nK,nI,nJ,nF,nc,nr,kcK,
%         icI,jcJ,jrJ,fcF,dcb,drib,y,b,ldc,ldri,lg,lal)
%       Not intended to be called by the user.

%   Copyright 1991-2009 The MathWorks, Inc.

 function [bnds,pert,dc,dri,dcb,drib,dcd,drid,g,y,b,sens] = ...
        grub(M,K,I,J,opt,numits,contol,Kc,Kr,Ic,Ir,Jc,Jr,Fc,Fr,...
                kc,kr,ic,ir,jc,jr,fc,fr,sc,sr,csc,csr,csF,nK,nI,nJ,nF,...
                nc,nr,kcK,icI,jcJ,jrJ,fcF,dcb,drib,y,b,ldc,ldri,lg,lal)

%       Authors:  Matthew P. Newlin and Peter M. Young


if nargin < 39
    disp('   grub.m called incorrectly')
    return
end
...
Mold = M;
[dcb,drib,M,warflag] = ...
    deebal(M,dcbold,dribold,Kc,Kr,Fc,Fr,kcK,nK,nJ,nF,csc,csr,csF,...
	jc,jr,sc,sr,opt,warflag);
% ----------------- BEGIN: mod per PMY for DKGIT ------------------------ %
% New addition to RUB to return SENS

nT = nK+nI+nJ;          sens = zeros(1,nT);
kijc = [kc;ic;jc];      kijr = [kr;ir;jr];
if nT > 1
   for ii = 1:nT
      sens(ii) = norm(M(logical(~kijc(ii,:)),logical(kijr(ii,:)))) + ...
		 norm(M(logical(kijc(ii,:)),logical(~kijr(ii,:))));
   end % for ii
end % if nT

% ----------------- END: mod per PMY for DKGIT ------------------------ %
scale = max(max(abs(M)))/5; if scale < 10*eps, scale = 1; else, M = M/scale; end
if isnan(scale) | isinf(scale),    error('   Data contains Inf or NaN'),  end
%%%%%%%%%%  lower bound
if exist('y')
