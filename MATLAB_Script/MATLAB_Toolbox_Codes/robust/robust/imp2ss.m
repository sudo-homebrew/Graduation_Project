...
%
% [G,TOTBND,SVH] = IMP2SS(Y) or
% [G,TOTBND,SVH] = IMP2SS(Y,TS,NU,NY,TOL) or
% [G,TOTBND,SVH] = IMP2SS(IMP_,TOL) produces an approximate
...
%  OUTPUTS: G = ss(A,B,C,D) continuous time state-space realization
...
ubar = u1'*u2;
a = ubar.*(invss*sqs');   % A = INV(DIAG(SS))*UBAR*DIAG(SS)
b = (sqs*ones(1,nu)).*v1(1:nu,:)';  % B = DIAG(SS)*V1(1:NU,:)'
c = u1(1:ny,:).*(ones(ny,1)*sqs');  % C = U1(1:NY,:)*DIAG(SS)
%
if ts>0,
