%function [k,gfin]=
%	sdhinfsyn(sdsys,nmeas,ncon,gmin,gmax,tol,h,delay,ricmethd,epr,epp,quiet)
%
...
%	             |__|
%
%   See also: H2SYN, HINFSYN, NORM, SDHINFNORM and SDLSIM
function [k,gfin]= ...
   xsdhinfsyn(sdsys,nmeas,ncon,gmin,gmax,tol,h,delay,ricmethd,epr,epp,quiet)
...
if isa(sdsys,'ss') || isa(sdsys,'tf') || isa(sdsys,'zpk')
   if get(sdsys,'Ts')~=0
...
while abs(gold-gtmp)>tol/2
   ...
   if hcompnorm==1
      [Ads,Bds,Cds,Dds]=rctutil.sdequiv(a,b1,b2,c1,c2,T,invT,S,s,h,delay);
      [dsdsysp,dsdsysm]=size(Dds);
      dsdsysn = size(Ads,1);
      
      % rescale gamma back from one to gtmp
      if dsdsysn>0
         idx1 = 1:dsdsysm-ncon;
         Bds(:,idx1) = Bds(:,idx1)*sqrt(gtmp);
         Dds(:,idx1) = Dds(:,idx1)*sqrt(gtmp);
         idx2 = 1:dsdsysp-nmeas;
         Cds(idx2,:) = Cds(idx2,:)*sqrt(gtmp);
         Dds(idx2,:) = Dds(idx2,:)*sqrt(gtmp);
         dsdsys = ss(Ads,Bds,Cds,Dds,h);
         [ktmp,kcl,kgam] = hinfsyn(dsdsys,nmeas,ncon,...
            'GMAX',gtmp,'GMIN',gtmp,'TOLGAM',tol,'DISPLAY',...
            display,'METHOD','ric'); % ,'EPP',epp,'EPR',epr);
         % XXXXMINOR
         % tmp = pck(dsdsys.a,dsdsys.b,dsdsys.c,dsdsys.d);
         % [ktmp]= dhfsyn(tmp,nmeas,ncon,gtmp,gtmp,tol,h,inf,...
         %                 dquiet,ricmethd,epr,epp);
         % [ak,bk,ck,dk]=unpck(ktmp);
         % ktmp = ss(ak,bk,ck,dk,h);
         % XXXX
      end %if if strcmp(dsdsystype,'syst')
      if dsdsysn==0 || isempty(ktmp)
         soln=0;
      else
         soln=1;
      end %if isempty(ktmp)
