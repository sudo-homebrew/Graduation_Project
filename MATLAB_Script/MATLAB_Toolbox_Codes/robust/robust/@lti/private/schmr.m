function [Gm,totbnd,hhsv] = schmr(G,mrtype,no)
%SCHMR Schur balanced truncation (unstable plant).
%
% [SS_M,TOTBND,HSV] = SCHMR(SS_,MRTYPE,NO) performs Schur method
...
%
if m == ra
   [Gm,augl,hhsv,slbig,srbig] = schbal(G,mrtype,no);
   augr = [0 0 0];
   totbnd = augl(1,2);
end
%
...
%
if m == 0
   [Gm,augr,hhsv,slbig,srbig] = schbal(-G,mrtype,no);
   Gm = -Gm;
   augl = [0 0 0];
   totbnd = augr(1,2);
end
...
if m > 0 & m < ra
  ...
  if mrtype == 1
   ...
   [Glf,augl,hsvl,tll,tlr] = schbal(Gl,mrtype,nol);
   if mrtype == 3
      disp('For the Unstable Part:');
   end
   [Grf,augr,hsvr,trl,trr] = schbal(-Gr,mrtype,nor);
   Grf = -Grf;
