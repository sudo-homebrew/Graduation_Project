function [Gred,info] = schurmr(G,varargin)
%SCHURMR  Schur balanced truncation model reduction for continuous/discrete 
%         & stable/unstable plant.
% 
%[GRED,REDINFO] = SCHURMR(G,ORDER,KEY1,VALUE1,KEY2,VALUE2,...) performs 
%    Schur balanced truncation model reduction on LTI system G such that the 
...
%    REDINFO.UnstabSV
%
% See also	REDUCE, BALANCMR, BSTMR, NCFMR, HANKELMR. 
...
if hasInternalDelay(G)    
    error(message('Robust:analysis:schurmr1'));
end

try 
    G = ss(G);
catch    
    error(message('Robust:analysis:schurmr2'));
end

if ndims(G) > 2
    error(message('Robust:analysis:schurmr3'));
end
...
if ~isempty(vvaagg)
   if mod(length(vvaagg),2) 
     error(message('Robust:analysis:schurmr4'));
   end
end
...
for i = 1:2:length(vvaagg)-1
    ...
    if ~isstr(curarg)
        error(message('Robust:analysis:schurmr5'));
    end
   ...
   case 'wei'
        ...
        if length(find(real(pole(wtval{1,1}))> 0)) || length(find(real(pole(wtval{1,2}))>0)) ...
         || length(find(real(tzero(wtval{1,1}))>0)) || length(find(real(tzero(wtval{1,2}))>0))
            error(message('Robust:analysis:schurmr6'));
        end
        try
           G = ss(wtval{1,1}'\G/wtval{1,2}','exp');
        catch
           error(message('Robust:analysis:schurmr7'));
        end
        [G,Gus] = stabproj(G);
    ...
    otherwise
        error(message('Robust:analysis:schurmr8',curarg0));
    end
end % for loop
...
if displayflag == 1 || Type == 3
   ...
   if isempty(redval{1,1})
     ord = input(getString(message('Robust:analysis:schurmr9',num2str(no_unstable+no_jw_pole))));  
     if isempty(ord)
           ord = length(ssdata(Gorig));
           disp(getString(message('Robust:analysis:schurmr10')));  
     end
     close(gcf)
...
for i = 1:stabexe
   if redval{1,1}(i) < 0
      ...
      warning(message('Robust:analysis:schurmr11',redval{1,1}(i)+no_ns,no_ns,no_ns));
      redval{1,1}(i) = 0;
   end
   [gr,aug,hhsv] = schbal(G,Type,redval{1,1}(i));
   if wtflag % exists weighting
      ggrr = wtval{1,1}'*(gr+Gus)*wtval{1,2}';
      [gr,gtemp] = stabproj(ggrr);
      gr = gr + get(gtemp,'d');
   end
   gr = gr+P+Gjw; % add back the unstable part of G
   if Ts, % discrete case
