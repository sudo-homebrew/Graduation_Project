...
for j = 1:size(i_lmi,2)
   if LMI_term(4,i_lmi(j)) ~= 0
     lmiterm = [ lmiterm LMI_term(:,i_lmi(j)) ]; %#ok<*AGROW>
   else
...
for k = 1:size(LMI_var,2)
   ...
   if all(LMI_var(5:6,k)==[1;1]),
     ...
     tx = ['X',num2str(klb)];
     SymVar = any(LMI_var(2,k) == [1 31]);  % flags symmetric variable
     for i = 1:size(i_lmi,2)
        ...
        if ind_col == ind_row,
          if tca + tcb == 0,
            ...
            elseif tsym == 1
               newstr = [newstr,tx];
               if ~SymVar, newstr = [newstr,' + ',tx,'''']; end
            elseif tsym == -1
          ...
          elseif tca +tcb == 2
            if SymVar && tsym ~= -Inf,
               ...
               if ~SymVar, blck_str = [blck_str,'''']; end
        ...
        elseif ~change,
          ...
          if lmiterm(4,i_lmi(i))==klb && ~SymVar, 
             tx = [tx,''''];
          end
          if tca + tcb == 2
