...
%
%   EVALSYS = EVALLMI(LMISYS,DECVARS) evaluates the system of LMIs described
%   in LMISYS for some particular instance DECVARS of the vector of decision
%   variables. Note that DECVARS is typically the output of some LMI solver.
%   Use SHOWLMI to further retrieve the value of a particular LMI.
%
%   Output:
...
for lmi=LMI_set,
  ...
  if any(any(Cleft))
    newdata=[newdata;insize;insize;Cleft(:)];
    newt(:,jnewt)=[lmilb;1;1;0;jdata;ld];
    jdata=jdata+ld; jnewt=jnewt+1;
  end

  if any(any(Cright))
    newdata=[newdata;insize;insize;Cright(:)];
    newt(:,jnewt)=[-lmilb;1;1;0;jdata;ld];
    jdata=jdata+ld; jnewt=jnewt+1;
  end
