...
%
%   NEWSYS = SETMVAR(LMISYS,XID,XVAL) sets the matrix variable with 
%   identifier XID to the value XVAL. All LMI terms involving this matrix 
%   variable are evaluated and added to the constant terms.
%
%   For convenience, set XID to the identifier returned by
...
%     XID          identifier of the variable matrix to be
%                  set to X
%     XVAL         matrix value assigned to the matrix variable XID (a 
%                  scalar t is interpreted as t*I)
%   Output:
...
for v=svars,
  ...
  for t=lmitv,
    ...
    data(bdt+3:bdt+ldt)=data(bdt+3:bdt+ldt)+C(:);
...
for t=LMI_term,
  j=j+1;
  LMI_term(5,j)=LMI_term(5,j)-sum(deldat(3,deldat(2,:)<t(5)));
end
