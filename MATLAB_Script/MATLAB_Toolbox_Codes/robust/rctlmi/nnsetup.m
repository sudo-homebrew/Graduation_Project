...
for ct=1:NLMI
   ...
   elseif lneg==1,    % one scalar block
      blckdims(ind)=insize-sum(blckdims(blckdims>0));
   ...
   elseif sum(blckdims)~=insize,
      error('LMI #%d: the inner and outer factors have incompatible dimensions',...
         lmilb)
   end
   ...
   lmit1=lmit2(2,lmit2(1,:)==lmilb);   % lhs terms
   lmit2=lmit2(2,lmit2(1,:)==-lmilb);  % rhs terms
...
for j=1:NV
   ...
   % fill r+1 .. r+3
   if all(var(5:6)==1)     % scalar variable
      izs(j3+(4:6))=[0;0;ilast];
      if vtype>30
         vtype=1;
         % Watch for X = lmivar(3,-n) to represent -xn. Because the solvers
         % don't handle scalar variables with minus sign, store the sign 
         % separately and later combine it with sign for LHS vs. RHS term.
         VARSIGNS(j) = sign(var(7));
         var(7:9)=[1;1;0];
      end
   else
...
shift=5+3*NV; free_entries=0;
%disp(sprintf('guessed length of IZS: %d',length(izs)))

for j=1:NLMI
   lmi = LMI_set(:,j);
   jlb=lmi(1);      % label of this LMI as known in LMI_term
   rangt=lmi(4):lmi(5);
   
   lmit=LMI_term(:,rangt);
   lmit=lmit(:,abs(lmit(1,:))==jlb);
   ...
   for ct=1:numel(ixT)  % all var. terms in this LMI
      t = lmit(:,ixT(ct));
      ...
      % Side of the LMI. Terms A*(-xn)*B switch side to account for minus sign
      signt = VARSIGNS(abs(nvar)) * sign(t(1));
      ...
      if all(LMI_var(5:6,abs(nvar))==1)  % SCALAR VARIABLE
         
         [A,~,flag]=nncoeff(t,data,1);
   ...
   % LHS outer factor
   lhsoutf=lmit(:,lmit(1,:)==jlb & lmit(2,:)==0);
   ...
   if isempty(lhsoutf),		% NL unspecified
      ...
      elseif insize==outsize,     % NL = Identity
         ...
         error('LMI #%d: nonsquare l.h.s. outer factor is missing',jlb)
      end
   else
      N=nncoeff(lhsoutf,data,2); % returns N(:) (same as N' in row-first storage)
      lN=length(N);
   ...
   % RHS outer factor
   rhsoutf=lmit(:,lmit(1,:)==-jlb & lmit(2,:)==0);
   ...
   if isempty(rhsoutf),		% NR unspecified
      ...
      elseif insize==outsize,	% NR = Identity
         ...
         error('LMI #%d: nonsquare r.h.s. outer factor is missing',jlb)
      end
   else
