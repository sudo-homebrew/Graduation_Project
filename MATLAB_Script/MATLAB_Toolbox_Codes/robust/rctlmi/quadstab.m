...
%
%   [TAU,P] = QUADSTAB(PDS,OPTIONS) takes a parameter-dependent model PDS
%   with state equation
%          E(p) dx/dt = A(p)x + B(p)u
%   and seeks a single Lyapunov matrix Q > 0 such
%   that
%          A(p)*Q*E(p)' + E(p)*Q*A(p)'  < 0
%
%   for all values of the uncertain parameter vector p.  If such a matrix Q
%   is found, the Lyapunov function
%           V(x) = x'*P*x    with    P = inv(Q)
%   proves robust stability over the entire parameter range
...
%
%   See also  ROBUSTSTAB, PDLSTAB, DECAY, QUADPERF.
...
suffcond=0;  % =1 when only sufficient conditions available
