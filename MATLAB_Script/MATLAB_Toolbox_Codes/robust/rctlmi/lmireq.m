...
if strcmp(task,'load'),
  ...
  set(hdl(18),'str','  ');
  set(hdl(19),'callb',...
     ['ssztmp=strtrim(get(ZZZ_ehdl(18),''str''));' ...
      'if ~isempty(ssztmp),' ...
...
elseif strcmp(task,'addtcode'),
  ...
  if isempty(strtrim(addvcmd(:))) & get(hdl(11),'userd'),
      stop=char(get(hdl(11),'str'));
      if isempty(strtrim(stop(:))),
        parslerr(hdl,'The LMI variables are not fully specified');
        set(hdl(13),'value',0); return
      end
  ...
  addtcmd=' ';
  if ~isempty(strtrim(addvcmd)), addtcmd=parslsys(hdl,lminame,varlist); end
...
elseif strcmp(task,'writecode'),
  ...
  filename=strrep(strtrim(get(hdl(18),'str')),' ','');
...
elseif strcmp(task,'readcode'),
  ...
  while 1,
    ...
    if ~isstr(line), break,
    elseif length(buffer), line=[buffer , strtrim(line)]; buffer=[];
    end
    ll=length(line);
    if ll<8, line=[];
    ...
    elseif ~isempty(findstr(line,'lmivar')) | ...
       ...
       stop=strstack(stop,[' ' strtrim(line)]);
    elseif ~isempty(findstr(line,'getlmis')) | ...
          ~isempty(findstr(line,'lmiterm')) | ...
          ~isempty(findstr(line,'newlmi')),
       sbot=strstack(sbot,[' ' strtrim(line)]);
    end
  end
  fclose(fid);

  if isempty(strtrim(stop(:))) & isempty(strtrim(sbot(:)))
