...
%
%   [TYP,K,NV] = PVINFO(PV) takes an uncertain parameter vector PV created 
%   with PVEC and returns
%   * the type TYP of parameter uncertainty:
%       - 'box' for parameters independently taking values in interval 
%       - 'pol' for parameters taking values in a multi-dimensional polytope
%   * the number of parameters K
%   * in the polytopic case, the number NV of vertices defining the
%     polytopic parameter range
%
%   Box case:
%   ---------
%   [PMIN,PMAX,DPMIN,DPMAX] = PVINFO(PV,'par',J) returns the lower and upper
%   bounds on the range and rate of variation of the J-th parameter
%
%   Polytope case:
%   --------------
%   Vj = PVINFO(PV,'par',J)  returns the J-th vertex of the polytopic 
%   parameter range
%
%   P = PVINFO(PV,'eval',COORD) returns the parameter vector value given a
%   set of polytopic coordinates.   If PV = [PV1,...,PVk]  and  
%   COORD = [c1,...,ck], the result is P = c1*PV1 + ... + ck*PVk
%
%   See also  PVEC, PSYS.
