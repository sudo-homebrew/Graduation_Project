function [data, header] = rinexread(filename)
...
try
    [rawheader, rawdata] = nav.internal.rinex.readfile(fullfilename);
catch ME
    throwAsCaller(ME);
end
...
data = parseData(fileContents, rawdata, rawheader);
if (nargout > 1)
    header = parseHeader(fileContents, rawheader);
end
