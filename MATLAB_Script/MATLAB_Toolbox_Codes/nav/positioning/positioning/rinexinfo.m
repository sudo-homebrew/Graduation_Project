function info = rinexinfo(filename)
...
try
    rawheader = nav.internal.rinex.readfile(fullfilename);
catch ME
    throwAsCaller(ME);
end

listing = dir(fullfilename);

if (rawheader.FileType == 'N')
    fileContents = nav.internal.rinex.NavFileContents;
else % (rawheader.FileType == 'O')
    fileContents = nav.internal.rinex.ObsFileContents;
end
info = parseHeader(fileContents, rawheader);
info.FileName = string(listing.name);
info.FileSize = listing.bytes;
end
