classdef wheelEncoderBicycle < nav.internal.WheelEncoderInterface ...
...
%   reset            - Reset the states of the WHEELENCODERBICYCLE
%
...
%#codegen
    ...
    properties
        ...
        TicksPerRevolution = wheelEncoderBicycle.TICKS_PER_REVOLUTION_DEFAULT * ones(1, wheelEncoderBicycle.getNumWheels);
        ...
        % 0.35].
        WheelRadius = wheelEncoderBicycle.WHEEL_RADIUS_DEFAULT * ones(1, wheelEncoderBicycle.getNumWheels);
        ...
        WheelRadiusBias = wheelEncoderBicycle.WHEEL_RADIUS_BIAS_DEFAULT * ones(1, wheelEncoderBicycle.getNumWheels);
        ...
        WheelPositionAccuracy = wheelEncoderBicycle.WHEEL_POSITION_ACCURACY_DEFAULT * ones(1, wheelEncoderBicycle.getNumWheels);
        ...
        SlipRatio = wheelEncoderBicycle.SLIP_RATIO_DEFAULT * ones(1, wheelEncoderBicycle.getNumWheels);
    ...
    methods
        function set.TicksPerRevolution(obj, val)
            nav.internal.WheelEncoderInterface.validateTicksPerRevolution(val, wheelEncoderBicycle.getNumWheels);
            obj.TicksPerRevolution(:) = val(:);
        end
        function set.WheelRadius(obj, val)
            nav.internal.WheelEncoderInterface.validateWheelRadius(val, wheelEncoderBicycle.getNumWheels);
            obj.WheelRadius(:) = val(:);
        end
        function set.WheelRadiusBias(obj, val)
            nav.internal.WheelEncoderInterface.validateWheelRadiusBias(val, wheelEncoderBicycle.getNumWheels);
            obj.WheelRadiusBias(:) = val(:);
        end
        function set.WheelPositionAccuracy(obj, val)
            nav.internal.WheelEncoderInterface.validateWheelPositionAccuracy(val, wheelEncoderBicycle.getNumWheels);
            obj.WheelPositionAccuracy(:) = val(:);
        end
        function set.SlipRatio(obj, val)
            nav.internal.WheelEncoderInterface.validateSlipRatio(val, wheelEncoderBicycle.getNumWheels);
            obj.SlipRatio(:) = val(:);
        end
    ...
    methods (Access = protected)
        function wheelDistance = vehicleSpeedToWheelDistance(obj, speed, omega)
            ...
            frontWheelVel = [speed, zero, zero] ...
                + cross([zero, zero, omega], [wheelBase, zero, zero]);

            frontWheelSpeed = vecnorm(frontWheelVel, 2, 2);
            wheelDistance = dt*[speed, frontWheelSpeed];
        end
