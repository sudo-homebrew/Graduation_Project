...
class Zigzag(Node):
    ...
    *******************************************************************************"""
    ...
    def process(): # init left down (-2.0, 2.0) upward direction, while steping step size % 4
        steping = True
        count = 0
        while steping:
            forwaring()
            steping = turning(count)
            count = count + 1
    ...
    def turning(count): # get LSD(only for forward distance in this case)
        if count % 4 == 0:
            robot_moving(4) # right90
            robot_moving(2, 3) # forwarding 0.45(approximate robot size)
            checking_wall()           
    ...
    def checking_wall():
        if self.min_obstacle_distance < 0.3:  # unit: m            
            return False
        return True

    def robot_moving(action=2, time=0.1): # action 0, 1, 2, 3, 4 left90, left45, forward, right45, right90
        twist = Twist()
        twist.linear.x = 0.15
        twist.angular.z = ((self.action_size - 1)/2 - action) * 1.5
        self.cmd_vel_pub.publish(twist)
